        ...
        <div class="flex items-center justify-between gap-2">
          <div class="flex-1 min-w-0 flex items-center gap-2 select-none">
            <div class="font-medium text-sm truncate flex items-center gap-2">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-primary flex-shrink-0 transition-transform duration-300" :class="{ 'rotate-90': !task.collapsed }" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <!-- Collapsed Info: Filename -->
             <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
              <div class="text-xs text-base-content/60 flex-shrink-0 truncate max-w-[150px]" v-if="task.collapsed && task.filePath">
          ...
          <div class="overflow-hidden min-h-0">
            <div class="flex flex-col gap-3 ml-6 pt-1 pb-2">
              ...
              <div class="flex gap-2 items-center" @click.stop>
                <span class="text-xs text-base-content/60 flex-shrink-0 w-20 text-right">{{ $t('readFlash.filePath') }}:</span>
              ...
              <!-- Start Address -->
              <div class="flex gap-2 items-center" @click.stop>
                <span class="text-xs text-base-content/60 flex-shrink-0 w-20 text-right">{{ $t('readFlash.address') }}:</span>
                <div class="flex-1">
                  <input 
                    type="text" 
              ...
              <!-- Read Size -->
              <div class="flex gap-2 items-center" @click.stop>
                <span class="text-xs text-base-content/60 flex-shrink-0 w-20 text-right">{{ $t('readFlash.size') }}:</span>
                <div class="flex-1">
                  <input 
                    type="text" 
                    v-model="task.size"
