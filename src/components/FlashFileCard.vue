          ...
          <div class="flex-1 min-w-0 flex items-center gap-2 select-none">
            <div class="font-medium text-sm truncate flex items-center gap-2">
              <svg
                ...
                :class="{ 'rotate-90': !file.collapsed }"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
              >
        ...
        >
          <div class="overflow-hidden min-h-0">
            <div class="flex flex-col gap-3 ml-6 pt-1">
              ...
              <!-- Expanded info: Path -->
              <div
                class="text-xs text-base-content/70 font-mono bg-base-200/50 border border-base-300/50 rounded px-2 py-1.5 truncate select-all"
                :title="file.path"
                @click.stop
              >
                {{ file.path }}
              ...
              <div class="flex gap-2 items-center" v-if="!store.isAutoAddressFile(file.name)" @click.stop>
                <span class="text-xs text-base-content/60 flex-shrink-0 w-16 text-right"
                  >{{ $t('writeFlash.address') }}:</span
                >
                <div class="flex-1">
                  <input
                    type="text"
              ...
              <!-- Address error -->
              <div
                v-if="file.addressError && !store.isAutoAddressFile(file.name)"
                class="text-xs text-error flex items-center gap-1 pl-[4.5rem]"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-3 w-3"
...
const validateAddress = () => {
  ...
  const duplicateIndex = store.selectedFiles.findIndex(
    (f, i) => i !== index && f.address === file.address && f.address
  );
  if (duplicateIndex !== -1) {
