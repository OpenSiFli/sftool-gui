      ...
      <div class="flex gap-2">
          ...
          <div class="relative">
            ...
            >
              ...
              >
                ...
                <div v-if="filteredChips.length === 0" class="p-2 text-gray-500 text-center text-xs">
                  {{ t('deviceConnection.noChipFound') }}
                </div>
              </div>
            </transition>
          </div>
        ...
        <div class="form-control w-1/2" v-if="selectedChip">
          ...
          <div class="relative">
            ...
            <div
              class="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-gray-500"
              @click="toggleMemoryTypeDropdown"
            >
              <span
                class="material-icons transform transition-transform duration-300 text-sm"
                :class="{ 'rotate-180': showMemoryTypeDropdown }"
              >
                keyboard_arrow_down
              </span>
            </div>
            ...
            >
              ...
              >
                ...
                <div v-if="availableMemoryTypes.length === 0" class="p-2 text-gray-500 text-center text-xs">
                  {{ t('deviceConnection.noStorageTypeAvailable') }}
                </div>
              </div>
            </transition>
          </div>
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <div class="form-control grow min-w-[120px]">
            <label class="label py-1">
              ...
              <button
                @click="refreshPorts"
                class="btn btn-xs btn-ghost transition-all duration-300"
                :disabled="isConnected || isConnecting"
              >
            ...
            <div class="relative">
              ...
              <div
                class="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-gray-500"
                @click="togglePortDropdown"
              >
                <span
                  class="material-icons transform transition-transform duration-300 text-sm"
                  :class="{ 'rotate-180': showPortDropdown }"
                >
                  keyboard_arrow_down
                </span>
              </div>
              ...
              >
                ...
                >
                  ...
                  <div v-else-if="availablePorts.length === 0" class="p-2 text-gray-500 text-center">
                    {{ t('deviceConnection.noPortFound') }}
                  </div>
                  <div
          ...
          <div class="form-control grow min-w-[120px]">
            ...
            <div class="relative">
              ...
              <div
                class="absolute right-3 top-1/2 -translate-y-1/2 cursor-pointer text-gray-500"
                @click="toggleBaudRateDropdown"
              >
                <span
                  class="material-icons transform transition-transform duration-300 text-sm"
                  :class="{ 'rotate-180': showBaudRateDropdown }"
                >
                  keyboard_arrow_down
                </span>
              </div>
...
const latestStatusEntry = computed(() => getLatestLogStatusEntry(logStore.entries));

const getStatusTextClass = () => {
  const latestEntry = latestStatusEntry.value;
  if (latestEntry?.level === 'error') {
    return 'text-error font-medium';
  } else if (logStore.isFlashing) {
    return 'text-success font-medium';
  } else if (latestEntry?.level === 'success') {
    return 'text-success font-medium';
  } else {
    return 'text-info';
  }
...
const getStatusText = () => {
  ...
  if (logStore.latestStatusText) {
    return t('deviceConnection.status') + ': ' + logStore.latestStatusText;
  }

  return t('deviceConnection.noLogMessages');
};
