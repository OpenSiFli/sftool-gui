            ...
            <div class="mb-4 flex-1 min-h-0 overflow-y-auto scrollable-container">
              ...
              >
                <div 
                  ...
                  :class="[getFileCardClass(file), { 'hover:shadow-md': file.collapsed }]"
                  @mouseleave="writeFlashStore.setFileCollapsed(index, true)"
                  @click="writeFlashStore.setFileCollapsed(index, false)"
                >
                          ...
                          <div class="font-medium text-sm truncate flex items-center gap-2">
                            ...
                            <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-x-2" enter-to-class="opacity-100 translate-x-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-x-0" leave-to-class="opacity-0 -translate-x-2">
                              <span v-if="file.collapsed" class="flex items-center gap-2 flex-shrink-0">
                                <span v-if="!writeFlashStore.isAutoAddressFile(file.name) && file.address" class="text-xs text-base-content/60 font-mono">
                                  @ {{ file.address }}
                                </span>
                                <span v-if="writeFlashStore.isAutoAddressFile(file.name)" class="badge badge-success badge-xs gap-0.5">
                      ...
                      >
                        <div class="overflow-hidden min-h-0">
                          <div class="flex flex-col gap-3 ml-6 pt-1">
                            ...
                            <!-- 地址设置 -->
                            <div class="flex gap-2 items-center" v-if="!writeFlashStore.isAutoAddressFile(file.name)" @click.stop>
...
const getFileCardClass = (file: FlashFile) => {
  ...
  // 检查当前是否正在烧录这个文件
  if (writeFlashStore.currentFlashingFile === fileName) {
    return 'downloading-file';
  }
  
  // 检查文件是否已完成
  if (writeFlashStore.completedFiles.has(fileName)) {
...
const validateAddress = (index: number) => {
  const file = writeFlashStore.selectedFiles[index];
  if (!file.address) {
    writeFlashStore.updateFileAddressError(index, t('writeFlash.validation.addressRequired'));
    return false;
  }
  
  const hexPattern = /^0x[0-9a-fA-F]+$/;
  if (!hexPattern.test(file.address)) {
    writeFlashStore.updateFileAddressError(index, t('writeFlash.validation.invalidAddress'));
    return false;
  }
  
  const addressValue = parseInt(file.address, 16);
  if (addressValue > 0xFFFFFFFF) {
    writeFlashStore.updateFileAddressError(index, t('writeFlash.validation.addressTooLarge'));
    return false;
  }
  
  const duplicateIndex = writeFlashStore.selectedFiles.findIndex((f, i) => 
    i !== index && f.address === file.address && f.address
  );
  if (duplicateIndex !== -1) {
    writeFlashStore.updateFileAddressError(index, t('writeFlash.validation.duplicateAddress'));
    return false;
  }
...
const removeFile = (index: number) => {
  const file = writeFlashStore.selectedFiles[index];
  writeFlashStore.removeFile(index);
