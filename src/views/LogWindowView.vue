<template>
  <div class="h-screen bg-base-100 flex flex-col overflow-hidden">
    <!-- 窗口头部 -->
    <div
      class="flex items-center justify-between p-4 border-b border-base-300 bg-gradient-to-r from-primary/5 to-primary/10 flex-shrink-0"
    >
      <div class="flex items-center gap-3">
        <div
          class="w-3 h-3 rounded-full"
          :class="logStore.isFlashing ? 'bg-success animate-pulse' : 'bg-base-300'"
        ></div>
      ...
      <div class="flex items-center gap-2">
        ...
        <div class="dropdown dropdown-bottom dropdown-end">
          ...
          <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-40">
            <li>
              <a @click="setLogLevel('all')" :class="{ active: logLevel === 'all' }"> 显示全部 </a>
            </li>
            <li>
              <a @click="setLogLevel('info')" :class="{ active: logLevel === 'info' }"> 仅信息 </a>
            </li>
            <li>
              <a @click="setLogLevel('error')" :class="{ active: logLevel === 'error' }"> 仅错误 </a>
            </li>
          </ul>
        </div>
        ...
        <div class="tooltip tooltip-bottom" data-tip="清除日志">
          <button class="btn btn-sm btn-circle btn-ghost" @click="clearLogs" :disabled="logStore.isFlashing">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
              fill="none"
        ...
        <div class="tooltip tooltip-bottom" data-tip="导出日志">
          <button class="btn btn-sm btn-circle btn-ghost" @click="exportLogs">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
              fill="none"
    ...
    <!-- 窗口底部状态栏 -->
    <div
      class="flex items-center justify-between px-4 py-3 border-t border-base-300 bg-base-50 text-xs text-base-content/60 flex-shrink-0"
    >
      <div class="flex items-center gap-4">
        <span>总消息: {{ logEntries.length }}</span>
        <span v-if="logLevel !== 'all'">已过滤: {{ filteredLogEntries.length }}</span>
      </div>
